...
public class FabricAutoRegisterHelper implements IAutoRegisterHelper {
        ...
        annotatedClasses.forEach(clazz -> {
            ...
            Arrays.stream(clazz.getDeclaredFields())
                ...
                .forEach(field -> {
                    ...
                    String name = field
                        .getAnnotation(AutoRegister.class)
                        .value();
                    Object o;
                    try {
                        o = field.get(null);
                    ...
                    AutoRegisterField autoRegisterField = new AutoRegisterField(
                        o,
                        identifier
                    );
    ...
    public void invokeAllAutoRegisterMethods(String packageName) {
        ...
        annotatedClasses.forEach(clazz ->
            Arrays.stream(clazz.getDeclaredMethods())
                .filter(method ->
                    method.isAnnotationPresent(AutoRegister.class)
                )
                .forEach(method -> {
                    ...
                    try {
                        ...
                        YungsApiCommon.LOGGER.error(
                            "Unable to invoke method {} - make sure it's static and has no args!",
                            method.getName()
                        );
                        throw new RuntimeException(e);
                    }
    ...
    public void registerBrewingRecipe(
        Supplier<Potion> inputPotion,
        Supplier<Item> ingredient,
        Supplier<Potion> outputPotion
    ) {
        FabricBrewingRecipeRegistryBuilder.BUILD.register(builder ->
            builder.addMix(
                Holder.direct(inputPotion.get()),
                ingredient.get(),
                Holder.direct(outputPotion.get())
            )
        );
    }

    @Override
    public void addCompostableItem(
        Supplier<Item> ingredient,
        float compostChance
    ) {
