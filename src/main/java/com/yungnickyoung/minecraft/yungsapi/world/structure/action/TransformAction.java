...
public class TransformAction extends StructureAction {
    ...
    > TEMPLATE_CODEC = Codec.of(
        TransformAction::encodeTemplate,
        Identifier.CODEC.map(Either::left)
    );

    public static final MapCodec<TransformAction> CODEC =
        RecordCodecBuilder.mapCodec(builder ->
            builder
                .group(
                    TEMPLATE_CODEC.listOf()
                        .fieldOf("output")
                        .forGetter(action -> action.output),
                    Codec.INT.optionalFieldOf("x_offset", 0).forGetter(action ->
                        action.xOffset
                    ),
                    Codec.INT.optionalFieldOf("y_offset", 0).forGetter(action ->
                        action.yOffset
                    ),
                    Codec.INT.optionalFieldOf("z_offset", 0).forGetter(action ->
                        action.zOffset
                    )
                )
    ...
    private static <T> DataResult<T> encodeTemplate(
        ...
        return either.left().isEmpty()
            ? DataResult.error(() ->
                  "yungsapi - Cannot serialize a runtime pool element"
              )
    ...
    public TransformAction(
        List<Either<Identifier, StructureTemplate>> output,
        int xOffset,
        int yOffset,
        int zOffset
    ) {
    ...
    public void apply(StructureContext ctx, PieceEntry targetPieceEntry) {
        ...
        if (templateManager == null) {
            YungsApiCommon.LOGGER.error(
                "Missing required field 'structureTemplateManager' for transform action!"
            );
            return;
        }

        // Transform piece, copying over most other data
        YungJigsawSinglePoolElement old =
            (YungJigsawSinglePoolElement) targetPieceEntry
                .getPiece()
                .getElement();
        ...
        Either<Identifier, StructureTemplate> newTemplate = this.output.get(
            rand.nextInt(this.output.size())
        );
        StructurePoolElement newElement = new YungJigsawSinglePoolElement(
            newTemplate,
            old.processors,
            old.getProjection(),
        ...
        BlockPos newPos = targetPieceEntry
            .getPiece()
            .getPosition()
            .offset(offset);

        // New piece bounding box
        BoundingBox newBoundingBox = newElement.getBoundingBox(
            templateManager,
            newPos,
        ...
        targetPieceEntry
            .getBoxOctree()
            .getValue()
            .removeBox(targetPieceEntry.getPieceAabb());
