...
public class JigsawManager {
    ...
    public static Optional<Structure.GenerationStub> assembleJigsawStructure(
        ...
        Registry<StructureTemplatePool> registry = registryAccess.lookupOrThrow(
        ...
        Optional<PoolElementStructurePiece> startPieceOptional = getStartPiece(
            startPool,
        ...
        int yAdjustment =
            pieceBoundingBox.minY() + startPiece.getGroundLevelDelta();
        ...
        return Optional.of(
            new Structure.GenerationStub(
                ...
                structurePiecesBuilder -> {
                    if (maxDepth <= 0) {
                        // Realistically this should never be true. Why make a jigsaw config with a non-positive size?
                        return;
                    }

                    // Create assembler + initial entry
                    JigsawStructureAssembler assembler =
                        new JigsawStructureAssembler(
                    ...
                    assembler.addAllPiecesToStructureBuilder(
                        structurePiecesBuilder
                    );
                }
            )
    ...
    private static Optional<PoolElementStructurePiece> getStartPiece(
        ...
        > candidatePoolElements = new ObjectArrayList<>(
        ...
        while (candidatePoolElements.size() > 0 && totalWeightSum > 0) {
            ...
            if (startJigsawNameOptional.isPresent()) {
                ...
                Optional<BlockPos> optional = getPosOfJigsawBlockWithName(
                    chosenPoolElement,
                    name,
                    locatePos,
                    rotation,
            ...
            if (
                chosenPoolElement instanceof YungJigsawPoolElement yungElement
            ) {
    ...
    private static Optional<BlockPos> getPosOfJigsawBlockWithName(
        ...
        try {
            List<StructureTemplate.JigsawBlockInfo> shuffledJigsawBlocks =
            ...
            YungsApiCommon.LOGGER.error(
                "Ignoring - the structure will still generate, but /locate will not point to the structure's anchor block."
            );
            return Optional.empty();
        }
