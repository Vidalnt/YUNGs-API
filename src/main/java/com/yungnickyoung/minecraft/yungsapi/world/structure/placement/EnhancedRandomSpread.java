...
public class EnhancedRandomSpread extends RandomSpreadStructurePlacement {
    ...
    public static final MapCodec<EnhancedRandomSpread> CODEC =
        RecordCodecBuilder.<EnhancedRandomSpread>mapCodec(builder ->
            builder
                .group(
                    Vec3i.offsetCodec(16)
                        .optionalFieldOf("locate_offset", Vec3i.ZERO)
                        .forGetter(placement -> placement.locateOffset()),
                    ...
                    Codec.floatRange(0.0F, 1.0F)
                        ...
                        .forGetter(placement -> placement.frequency()),
                    ExtraCodecs.NON_NEGATIVE_INT.fieldOf("salt").forGetter(
                        placement -> placement.salt()
                    ),
                    ExclusionZone.CODEC.optionalFieldOf(
                        "exclusion_zone"
                    ).forGetter(placement -> placement.exclusionZone()),
                    EnhancedExclusionZone.CODEC.optionalFieldOf(
                        "enhanced_exclusion_zone"
                    ).forGetter(placement -> placement.enhancedExclusionZone),
                    ExtraCodecs.NON_NEGATIVE_INT.fieldOf("spacing").forGetter(
                        RandomSpreadStructurePlacement::spacing
                    ),
    ...
    private static DataResult<EnhancedRandomSpread> validateSpacing(
        EnhancedRandomSpread placement
    ) {
        return placement.spacing() <= placement.separation()
            ? DataResult.error(() ->
                  "EnhancedRandomSpread's spacing has to be larger than separation"
              )
    ...
    public boolean isStructureChunk(
        ...
        return (
            this.enhancedExclusionZone.isEmpty() ||
            !this.enhancedExclusionZone.get().isPlacementForbidden(
