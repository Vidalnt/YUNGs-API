...
public class EnhancedBeardifierHelper {
    ...
    public static Beardifier forStructuresInChunk(
        ...
        ObjectList<EnhancedBeardifierRigid> enhancedBeardifierRigidList =
            new ObjectArrayList<>(10);
        ObjectList<EnhancedJigsawJunction> enhancedJunctionList =
            new ObjectArrayList<>(10);
        ...
        List<StructureStart> structureStarts =
            structureManager.startsForStructure(chunkPos, structure ->
                structure instanceof YungJigsawStructure
            );
        for (StructureStart structureStart : structureStarts) {
            EnhancedTerrainAdaptation structureTerrainAdaptation = (
                (YungJigsawStructure) structureStart.getStructure()
            ...
            List<StructurePiece> nearbyPieces = structureStart
                .getPieces()
                .stream()
                .filter(structurePiece ->
                    structurePiece.isCloseToChunk(chunkPos, maxKernelRadius)
                )
                .toList();

            for (StructurePiece nearbyPiece : nearbyPieces) {
                if (
                    nearbyPiece instanceof
                        PoolElementStructurePiece poolElementPiece
                ) {
                    StructureTemplatePool.Projection projection =
                        poolElementPiece.getElement().getProjection();

                    // Check if piece overrides terrain adaptation
                    EnhancedTerrainAdaptation pieceTerrainAdaptation =
                        structureTerrainAdaptation;
                    if (
                        poolElementPiece.getElement() instanceof
                            YungJigsawPoolElement yungElement &&
                        yungElement.getEnhancedTerrainAdaptation().isPresent()
                    ) {
                        pieceTerrainAdaptation = yungElement
                            .getEnhancedTerrainAdaptation()
                            .get();
                    }
                    ...
                    if (
                        pieceTerrainAdaptation == EnhancedTerrainAdaptation.NONE
                    ) continue;

                    int pieceKernelRadius =
                        pieceTerrainAdaptation.getKernelRadius();
                    ...
                    for (JigsawJunction jigsawJunction : poolElementPiece.getJunctions()) {
                        ...
                        if (
                            sourceX > chunkMinBlockX - pieceKernelRadius &&
                            ...
                            enhancedJunctionList.add(
                                new EnhancedJigsawJunction(
                                    jigsawJunction,
                                    pieceTerrainAdaptation
                                )
                            );
                    ...
                    structureTerrainAdaptation != EnhancedTerrainAdaptation.NONE
                ) {
                    enhancedBeardifierRigidList.add(
                        new EnhancedBeardifierRigid(
        ...
        BeardifierAccessor accessor = (BeardifierAccessor) original;
        Beardifier newBeardifier = new Beardifier(
            accessor.getPieces(),
            accessor.getJunctions(),
            accessor.getAffectedBox()
        );
        EnhancedBeardifierData enhancedBeardifier =
            (EnhancedBeardifierData) newBeardifier;
        enhancedBeardifier.setEnhancedPieceIterator(
            enhancedBeardifierRigidList.iterator()
        );
        enhancedBeardifier.setEnhancedJunctionIterator(
            enhancedJunctionList.iterator()
        );
    ...
    public static double computeDensity(
        ...
        while (
            data.getEnhancedPieceIterator() != null &&
            data.getEnhancedPieceIterator().hasNext()
        ) {
            EnhancedBeardifierRigid rigid = data
                .getEnhancedPieceIterator()
                .next();
            BoundingBox pieceBoundingBox = rigid.pieceBoundingBox();
            EnhancedTerrainAdaptation pieceTerrainAdaptation =
                rigid.pieceTerrainAdaptation();
            Rotation pieceRotation = rigid.rotation();

            // Apply properties from the piece's terrain adaptation to the bounding box:
            // - bottom offset
            pieceBoundingBox = pieceBoundingBox.moved(
                0,
                (int) pieceTerrainAdaptation.getBottomOffset(),
                0
            );

            // - x/z padding
            Direction.Axis xPaddingDirection = pieceRotation
                .rotate(Direction.EAST)
                .getAxis();
            int xPadding =
                xPaddingDirection == Direction.Axis.X
                    ? pieceTerrainAdaptation.getPadding().x()
                    : pieceTerrainAdaptation.getPadding().z();
            int zPadding =
                xPaddingDirection == Direction.Axis.X
                    ? pieceTerrainAdaptation.getPadding().z()
                    : pieceTerrainAdaptation.getPadding().x();
            pieceBoundingBox = pieceBoundingBox.inflatedBy(
                xPadding,
                0,
                zPadding
            );
            ...
            if (pieceTerrainAdaptation.getPadding().top() != 0) {
                pieceBoundingBox = new BoundingBox(
                    ...
                    pieceBoundingBox.maxY() +
                        pieceTerrainAdaptation.getPadding().top(),
            ...
            if (pieceTerrainAdaptation.getPadding().bottom() != 0) {
                pieceBoundingBox = new BoundingBox(
                    pieceBoundingBox.minX(),
                    pieceBoundingBox.minY() -
                        pieceTerrainAdaptation.getPadding().bottom(),
                    pieceBoundingBox.minZ(),
                    pieceBoundingBox.maxX(),
                    pieceBoundingBox.maxY(),
                    pieceBoundingBox.maxZ()
             ...
             */
            int xDistanceToBoundingBox = Math.max(
                0,
                Math.max(
                    pieceBoundingBox.minX() - x,
                    x - pieceBoundingBox.maxX()
                )
            );
            int yDistanceToBoundingBox = Math.max(
                0,
                Math.max(
                    pieceBoundingBox.minY() - y,
                    y - pieceBoundingBox.maxY()
                )
            );
            int zDistanceToBoundingBox = Math.max(
                0,
                Math.max(
                    pieceBoundingBox.minZ() - z,
                    z - pieceBoundingBox.maxZ()
                )
            );
            ...
            if (pieceTerrainAdaptation != EnhancedTerrainAdaptation.NONE) {
                densityFactor =
                    pieceTerrainAdaptation.computeDensityFactor(
            ...
            if (
                densityFactor != 0 &&
                pieceTerrainAdaptation.getAquiferOverride() !=
        ...
        // Vanilla logic
        while (
            data.getEnhancedJunctionIterator() != null &&
            data.getEnhancedJunctionIterator().hasNext()
        ) {
            EnhancedJigsawJunction enhancedJigsawJunction = data
                .getEnhancedJunctionIterator()
                .next();
            JigsawJunction jigsawJunction =
                enhancedJigsawJunction.jigsawJunction();
            EnhancedTerrainAdaptation pieceTerrainAdaptation =
                enhancedJigsawJunction.pieceTerrainAdaptation();
    ...
    private static void updateAquiferOverrideMask(
        EnhancedBeardifierData data,
        ...
        AquiferOverrideMaskSupplier supplier =
            (AquiferOverrideMaskSupplier) noiseChunk;

        int chunkHeight = supplier.noise_getHeight();
        int minY = supplier.noise_getMinY();

        AquiferOverrideMask aquiferOverrideMask =
            supplier.getOrCreateAquiferOverrideMask(() ->
                new AquiferOverrideMask(chunkHeight, minY)
            );
