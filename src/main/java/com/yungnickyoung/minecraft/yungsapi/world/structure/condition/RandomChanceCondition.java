...
public class RandomChanceCondition extends StructureCondition {
    ...
    public static final MapCodec<RandomChanceCondition> CODEC =
        RecordCodecBuilder.mapCodec(builder ->
            builder
                .group(
                    ExtraCodecs.POSITIVE_FLOAT.fieldOf("chance").forGetter(
                        condition -> condition.chance
                    )
                )
    ...
    public boolean passes(StructureContext ctx) {
        ...
        if (random == null) {
            YungsApiCommon.LOGGER.error(
                "Missing required field 'random' for random_chance condition!"
            );
            return false;
        }
