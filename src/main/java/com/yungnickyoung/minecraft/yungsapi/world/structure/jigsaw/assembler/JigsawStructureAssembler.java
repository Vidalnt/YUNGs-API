...
public class JigsawStructureAssembler {
     ...
     * has been called.
     */
    public void addAllPiecesToStructureBuilder(
    ...
    private void addChildrenForPiece(PieceEntry pieceEntry) {
        ...
        List<StructureTemplate.JigsawBlockInfo> pieceJigsawBlocks = piece
            .getElement()
        ...
        for (StructureTemplate.JigsawBlockInfo jigsawBlockInfo : pieceJigsawBlocks) {
            ...
            ResourceKey<StructureTemplatePool> poolKey = readPoolKey(
                jigsawBlockInfo
            );

            // Fetch the target pool, ensuring it's not empty
            Optional<
                ? extends Holder<StructureTemplatePool>
            > optionalPoolHolder = this.settings.poolRegistry.get(poolKey);
            if (optionalPoolHolder.isEmpty()) {
                YungsApiCommon.LOGGER.warn(
                    "Empty or nonexistent pool: {}",
                    poolKey.identifier()
                );
                continue;
            }
            Holder<StructureTemplatePool> targetPoolHolder =
                optionalPoolHolder.get();
            ...
            Holder<StructureTemplatePool> fallbackPoolHolder = targetPoolHolder
                .value()
                .getFallback();
            StructureTemplatePool fallbackPool = fallbackPoolHolder.value();
            if (
                fallbackPool.size() == 0 && !fallbackPoolHolder.is(Pools.EMPTY)
            ) {
                YungsApiCommon.LOGGER.warn(
                    "Empty or nonexistent fallback pool: {}",
                    fallbackPoolHolder
                        .unwrapKey()
            ...
            PieceContext pieceContext = createPieceContextForJigsawBlock(
                jigsawBlockInfo,
                pieceEntry,
                parentOctree
            );
            Optional<StructurePoolElement> newlyGeneratedPiece =
                Optional.empty();

            // Attempt to place a piece from the target pool
            if (pieceEntry.getDepth() != this.settings.maxDepth) {
                pieceContext.candidatePoolElements = new ObjectArrayList<>(
                    (
                        (StructureTemplatePoolAccessor) targetPool
                    ).getRawTemplates()
                );
                newlyGeneratedPiece = this.chooseCandidateFromPool(
                    pieceContext
                );
            }
            ...
            if (newlyGeneratedPiece.isEmpty()) {
                pieceContext.candidatePoolElements = new ObjectArrayList<>(
                    (
                        (StructureTemplatePoolAccessor) fallbackPool
                    ).getRawTemplates()
                );
                newlyGeneratedPiece = this.chooseCandidateFromPool(
                    pieceContext
         ...
         */
        if (
            pieceEntry.getDeadendPool().isPresent() &&
            !generatedAtLeastOneChildPiece &&
            pieceJigsawBlocks.size() > 1
        ) {
            // Get deadend pool from id
            Identifier deadendPoolId = pieceEntry.getDeadendPool().get();
            Optional<StructureTemplatePool> deadendPool =
                this.settings.poolRegistry.getOptional(deadendPoolId);
            if (deadendPool.isEmpty()) {
                YungsApiCommon.LOGGER.error(
                    "Unable to find deadend pool {} for element {}",
                    deadendPoolId,
                    piece.getElement()
                );
                return;
            ...
            newContext.candidatePoolElements = new ObjectArrayList<>(
                (
                    (StructureTemplatePoolAccessor) deadendPool.get()
                ).getRawTemplates()
            );
            ...
            if (parentEntry != null && pieceAabb != null) {
                ...
                if (
                    ...
                    this.pieceCounts.put(
                        pieceName,
                        this.pieceCounts.get(pieceName) - 1
                    );
                }
                ...
                if (
                    ...
                    this.pieceCounts.containsKey(
                        maxCountJigsawPoolElement.getName()
                    )
                ) {
    ...
    private PieceContext createPieceContextForJigsawBlock(
        ...
        Direction direction = JigsawBlock.getFrontFacing(
            jigsawBlockInfo.info().state()
        );
        BlockPos jigsawBlockTargetPos = jigsawBlockInfo
            .info()
            .pos()
            .relative(direction);

        // Adjustments for if the target block position is inside the current piece
        boolean isTargetInsideCurrentPiece = pieceBoundingBox.isInside(
     ...
     */
    private Optional<StructurePoolElement> chooseCandidateFromPool(
        PieceContext context
    ) {
        // Extract args from context
        ObjectArrayList<
            Pair<StructurePoolElement, Integer>
        > candidatePoolElements = context.candidatePoolElements;
        PoolElementStructurePiece piece = context.pieceEntry.getPiece();
        boolean isPieceRigid =
            piece.getElement().getProjection() ==
            StructureTemplatePool.Projection.RIGID;
        int jigsawBlockRelativeY =
            context.jigsawBlockPos.getY() - context.pieceMinY;
        ...
        int totalWeightSum = candidatePoolElements
            .stream()
        ...
        while (!candidatePoolElements.isEmpty() && totalWeightSum > 0) {
            ...
            for (Pair<
                ...
                Integer
            > candidatePiecePair : candidatePoolElements) {
                StructurePoolElement candidatePiece =
                    candidatePiecePair.getFirst();
                if (
                    candidatePiece instanceof
                        YungJigsawPoolElement yungElement &&
                    yungElement.isPriorityPiece()
                ) {
            ...
            if (chosenPoolElementPair == null) {
                ...
                int chosenWeight =
                    this.settings.rand.nextInt(totalWeightSum) + 1;

                // Randomly choose a candidate piece
                for (Pair<
                    StructurePoolElement,
                    Integer
                > candidate : candidatePoolElements) {
            ...
            StructurePoolElement chosenPoolElement =
                chosenPoolElementPair.getFirst();
            ...
            if (
                ...
                if (yungElement.name.isEmpty()) {
                    YungsApiCommon.LOGGER.error(
                        "Found YUNG Jigsaw piece with max_count={} missing \"name\" property.",
                        pieceMaxCount
                    );
                    YungsApiCommon.LOGGER.error(
                        "Max count pieces must be named in order to work properly!"
                    );
                    YungsApiCommon.LOGGER.error(
                        "Ignoring max_count for this piece..."
                    );
                } else {
                    String pieceName = yungElement.name.get();
                    ...
                    if (
                        this.maxPieceCounts.containsKey(pieceName) &&
                        this.maxPieceCounts.get(pieceName) != pieceMaxCount
                    ) {
                        YungsApiCommon.LOGGER.error(
                        ...
                        YungsApiCommon.LOGGER.error(
                            "This can happen when multiple pieces across pools use the same name, but have different max_count values."
                        );
                        YungsApiCommon.LOGGER.error(
                            "Please change these max_count values to match. Using max_count={} for now...",
                            pieceMaxCount
                        );
                    }
                    ...
                    if (
                        this.pieceCounts.getOrDefault(pieceName, 0) >=
                        pieceMaxCount
                    ) {
            ...
            if (chosenPoolElement instanceof IMaxCountJigsawPoolElement) {
                String pieceName = (
                    (IMaxCountJigsawPoolElement) chosenPoolElement
                ).getName();
                int maxCount = (
                    (IMaxCountJigsawPoolElement) chosenPoolElement
                ).getMaxCount();
                ...
                if (
                    this.maxPieceCounts.containsKey(pieceName) &&
                    this.maxPieceCounts.get(pieceName) != maxCount
                ) {
                    YungsApiCommon.LOGGER.error(
            ...
            if (
                chosenPoolElement instanceof
                    YungJigsawPoolElement yungElement &&
                !yungElement.isAtValidDepth(context.depth)
            ) {
                totalWeightSum -= chosenPieceWeight;
                candidatePoolElements.remove(chosenPoolElementPair);
                continue;
            ...
            for (Rotation rotation : Rotation.getShuffled(this.settings.rand)) {
                List<StructureTemplate.JigsawBlockInfo> candidateJigsawBlocks =
                    chosenPoolElement.getShuffledJigsawBlocks(
                        this.settings.structureTemplateManager,
                        BlockPos.ZERO,
                        rotation,
                        this.settings.rand
                ...
                BoundingBox tempCandidateBoundingBox =
                    chosenPoolElement.getBoundingBox(
                        this.settings.structureTemplateManager,
                        BlockPos.ZERO,
                        rotation
                    );
                ...
                if (
                    this.settings.useExpansionHack &&
                    tempCandidateBoundingBox.getYSpan() <= 16
                ) {
                    candidateHeightAdjustments = candidateJigsawBlocks
                        .stream()
                        .mapToInt(pieceCandidateJigsawBlock -> {
                            if (
                                !tempCandidateBoundingBox.isInside(
                            ...
                            > candidateTargetPoolKey = readPoolKey(
                                pieceCandidateJigsawBlock
                            );
                            Optional<
                                ? extends Holder<StructureTemplatePool>
                            > candidateTargetPool =
                                this.settings.poolRegistry.get(
                                    candidateTargetPoolKey
                                );
                            Optional<
                                Holder<StructureTemplatePool>
                            > candidateFallbackPool = candidateTargetPool.map(
                                poolHolder -> poolHolder.value().getFallback()
                            );
                            int candidateMaxSize = candidateTargetPool
                                .map(poolHolder ->
                                    poolHolder
                                        .value()
                                        .getMaxSize(
                                ...
                                .orElse(0);
                            int candidateFallbackMaxSize = candidateFallbackPool
                                .map(poolHolder ->
                                    poolHolder
                                        .value()
                                        .getMaxSize(
                ...
                for (StructureTemplate.JigsawBlockInfo candidateJigsawBlock : candidateJigsawBlocks) {
                    if (
                        !JigsawBlock.canAttach(
                            context.jigsawBlock,
                            candidateJigsawBlock
                        )
                    ) continue;

                    BlockPos candidateJigsawBlockPos = candidateJigsawBlock
                        .info()
                        .pos();
                    BlockPos candidateJigsawBlockRelativePos =
                        context.jigsawBlockTargetPos.subtract(
                            candidateJigsawBlockPos
                        );

                    // Get the rotated bounding box for the piece, offset by the relative position difference
                    BoundingBox rotatedCandidateBoundingBox =
                        chosenPoolElement.getBoundingBox(
                            this.settings.structureTemplateManager,
                            candidateJigsawBlockRelativePos,
                            rotation
                        );

                    // Determine if candidate is rigid
                    StructureTemplatePool.Projection candidateProjection =
                        chosenPoolElement.getProjection();
                    ...
                    int candidateJigsawBlockRelativeY =
                        candidateJigsawBlockPos.getY();
                    int candidateJigsawYOffsetNeeded =
                        jigsawBlockRelativeY -
                        candidateJigsawBlockRelativeY +
                        JigsawBlock.getFrontFacing(
                            context.jigsawBlock.info().state()
                        ).getStepY();
                    ...
                    if (isPieceRigid && isCandidateRigid) {
                        adjustedCandidatePieceMinY =
                            context.pieceMinY + candidateJigsawYOffsetNeeded;
                    } else {
                        if (surfaceHeight == -1) {
                            surfaceHeight =
                                this.settings.chunkGenerator.getFirstFreeHeight(
                        ...
                        adjustedCandidatePieceMinY =
                            surfaceHeight - candidateJigsawBlockRelativeY;
                    }
                    int candidatePieceYOffsetNeeded =
                        adjustedCandidatePieceMinY -
                        rotatedCandidateBoundingBox.minY();

                    // Offset the candidate's bounding box by the necessary amount
                    BoundingBox adjustedCandidateBoundingBox =
                        rotatedCandidateBoundingBox.moved(
                            0,
                            candidatePieceYOffsetNeeded,
                            0
                        );

                    // Add this offset to the relative jigsaw block position as well
                    BlockPos adjustedCandidateJigsawBlockRelativePos =
                        candidateJigsawBlockRelativePos.offset(
                            0,
                            candidatePieceYOffsetNeeded,
                            0
                        );
                    ...
                    if (candidateHeightAdjustments > 0) {
                        int k2 = Math.max(
                            candidateHeightAdjustments + 1,
                            adjustedCandidateBoundingBox.maxY() -
                                adjustedCandidateBoundingBox.minY()
                        );
                        (
                            (BoundingBoxAccessor) adjustedCandidateBoundingBox
                        ).setMaxY(adjustedCandidateBoundingBox.minY() + k2);
                    }

                    // Prevent pieces from generating outside the structure's boundaries
                    if (
                        !this.settings.isInBounds(
                            adjustedCandidateBoundingBox.maxY()
                        ) ||
                        !this.settings.isInBounds(
                            adjustedCandidateBoundingBox.minY()
                    ...
                    if (!pieceIgnoresBounds) {
                        boolean pieceIntersectsExistingPieces =
                            context.boxOctree
                                .getValue()
                                .intersectsAnyBox(aabbDeflated);
                        boolean pieceIsContainedWithinStructureBoundaries =
                            context.boxOctree
                                .getValue()
                                .boundaryContains(aabbDeflated);
                        if (
                            pieceIntersectsExistingPieces ||
                            !pieceIsContainedWithinStructureBoundaries
                        ) {
                            continue;
                        }
                    }
                    ...
                    if (isPieceRigid) {
                        candidateJigsawBlockY =
                            context.pieceMinY + jigsawBlockRelativeY;
                        ...
                        candidateJigsawBlockY =
                            surfaceHeight + candidateJigsawYOffsetNeeded / 2;
                    }
                    ...
                    PieceEntry newPieceEntry = new PieceEntry(
                        newPiece,
                        context.boxOctree,
                        aabb,
                        context.depth + 1,
                    ...
                    if (
                        ...
                        this.pieceCounts.put(
                            pieceName,
                            this.pieceCounts.getOrDefault(pieceName, 0) + 1
                        );
                    }

                    // LEGACY - support for IMaxCountJigsawPiece
                    if (
                        chosenPoolElement instanceof IMaxCountJigsawPoolElement
                    ) {
                        String pieceName = (
                            (IMaxCountJigsawPoolElement) chosenPoolElement
                        ).getName();
                ...
                if (yungElement.hasModifiers()) {
                    ...
                    StructureContext structureContext =
                        new StructureContext.Builder()
                            ...
                            .biomeSource(this.settings.biomeSource)
                            .build();
                    yungElement.modifiers.forEach(modifier ->
                        modifier.apply(structureContext)
                    );
                }
            }
        }
    ...
    public static class Settings {
        ...
        public Settings structureTemplateManager(
        ...
        public Settings levelHeightAccessor(
        ...
        public boolean isInBounds(int y) {
            ...
            if (
                y < levelHeightAccessor.getMinY() + dimensionPadding.bottom()
            ) return false;
            if (
                y > levelHeightAccessor.getMaxY() - dimensionPadding.top()
            ) return false;
            return true;
        }
    }
