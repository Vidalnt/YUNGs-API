...
public abstract class EnhancedTerrainAdaptation {
    ...
    EnhancedTerrainAdaptation(
        int kernelSize,
        int kernelDistance,
        TerrainAction topAction,
        TerrainAction bottomAction,
        double bottomOffset,
        Padding padding,
        ...
        this.kernel = Util.make(
            new float[kernelSize * kernelSize * kernelSize],
            kernel -> {
     ...
     */
    private float computeKernelValue(
        double xDistance,
        double yDistance,
        double zDistance
    ) {
        double squaredDistance = Mth.lengthSquared(
            xDistance,
            yDistance,
            zDistance
        );
    ...
    public double computeDensityFactor(
        ...
        if (
            isInKernelRange(kernelX) &&
            isInKernelRange(kernelY) &&
            isInKernelRange(kernelZ)
        ) {
            ...
            double actualYDistanceToPieceBottom =
                (double) yDistanceToPieceBottom + 0.5;
            ...
            double multiplier = Math.abs(
                (actualYDistanceToPieceBottom *
                        Mth.invSqrt(squaredDistance / 2.0)) /
                    2.0
            );

            // Get the density modifier for the action we are performing.
            boolean isAboveBeardBase = actualYDistanceToPieceBottom > 0;
            int densityModifier = isAboveBeardBase
                ? this.topAction.getDensityModifier()
                : this.bottomAction.getDensityModifier();
    ...
    public record Padding(int x, int top, int bottom, int z) {
        ...
        public static final Codec<Padding> CODEC = RecordCodecBuilder.create(
            instance ->
                instance
                    .group(
                        Codec.INT.optionalFieldOf("x", 0).forGetter(padding ->
                            padding.x
                        ),
                        Codec.INT.optionalFieldOf("top", 0).forGetter(padding ->
                            padding.top
                        ),
                        Codec.INT.optionalFieldOf("bottom", 0).forGetter(
                            padding -> padding.bottom
                        ),
                        Codec.INT.optionalFieldOf("z", 0).forGetter(padding ->
                            padding.z
                        )
                    )
    ...
    public enum TerrainAction implements StringRepresentable {
        ...
        public static final Codec<TerrainAction> CODEC =
            StringRepresentable.fromValues(TerrainAction::values);
