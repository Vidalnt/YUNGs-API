...
public class YungJigsawStructure extends Structure {
    ...
    public static final MapCodec<YungJigsawStructure> CODEC =
        RecordCodecBuilder.<YungJigsawStructure>mapCodec(builder ->
            builder
                .group(
                    ...
                    IntProvider.codec(0, 15)
                        .optionalFieldOf("x_offset_in_chunk", ConstantInt.of(0))
                        .forGetter(structure -> structure.xOffsetInChunk),
                    IntProvider.codec(0, 15)
                        .optionalFieldOf("z_offset_in_chunk", ConstantInt.of(0))
                        .forGetter(structure -> structure.zOffsetInChunk),
                    Codec.BOOL.optionalFieldOf(
                        "use_expansion_hack",
                        false
                    ).forGetter(structure -> structure.useExpansionHack),
                    Heightmap.Types.CODEC.optionalFieldOf(
                        "project_start_to_heightmap"
                    ).forGetter(structure -> structure.projectStartToHeightmap),
                    Codec.intRange(1, MAX_TOTAL_STRUCTURE_RADIUS)
                    ...
                    EnhancedTerrainAdaptationType.ADAPTATION_CODEC.optionalFieldOf(
                        "enhanced_terrain_adaptation",
                        EnhancedTerrainAdaptation.NONE
                    ).forGetter(structure ->
                        structure.enhancedTerrainAdaptation
                    ),
    ...
    private static DataResult<YungJigsawStructure> validateRange(
        YungJigsawStructure structure
    ) {
        if (
            ...
            return DataResult.error(() ->
                "YUNG Structure cannot use both vanilla terrain_adaptation and enhanced_terrain_adaptation"
            );
        }
        ...
        if (structure.maxDistanceFromCenter + vanillaEdgeBuffer > 128) {
            return DataResult.error(() ->
                "YUNG Structure's max_distance_from_center must not exceed 116 when using vanilla terrain_adaptation"
            );
        }
        ...
        int enhancedEdgeBuffer =
            structure.enhancedTerrainAdaptation.getKernelRadius();
    ...
    @Override
    public @NotNull Optional<GenerationStub> findGenerationPoint(
    ...
    @Override
    public @NotNull BoundingBox adjustBoundingBox(
        @NotNull BoundingBox boundingBox
    ) {
        return super
            .adjustBoundingBox(boundingBox)
