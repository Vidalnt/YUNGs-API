...
public class YungJigsawSinglePoolElement extends YungJigsawPoolElement {
    ...
    > TEMPLATE_CODEC = Codec.of(
        YungJigsawSinglePoolElement::encodeTemplate,
        Identifier.CODEC.map(Either::left)
    );

    public static final MapCodec<YungJigsawSinglePoolElement> CODEC =
        RecordCodecBuilder.mapCodec(builder ->
            builder
                .group(
                    ...
                    Identifier.CODEC.optionalFieldOf("deadend_pool").forGetter(
                        element -> element.deadendPool
                    ),
                    StructureModifier.CODEC.listOf()
                        .optionalFieldOf("modifiers", new ArrayList<>())
                        .forGetter(element -> element.modifiers)
                )
    ...
    > HIGHEST_SELECTION_PRIORITY_FIRST = Comparator.comparingInt(
    ...
    public Vec3i getSize(
        ...
        StructureTemplate structureTemplate = this.getTemplate(
    ...
    public List<StructureTemplate.JigsawBlockInfo> getShuffledJigsawBlocks(
        ...
        List<StructureTemplate.JigsawBlockInfo> jigsawBlocks = this.getTemplate(
            structureTemplateManager
        ).getJigsaws(blockPos, rotation);
        Util.shuffle(jigsawBlocks, randomSource);
        sortBySelectionPriority(jigsawBlocks);
        return jigsawBlocks;
    }

    private static void sortBySelectionPriority(
        List<StructureTemplate.JigsawBlockInfo> jigsawBlocks
    ) {
    ...
    public BoundingBox getBoundingBox(
        ...
        return structureTemplate.getBoundingBox(
    ...
    public boolean place(
        ...
        StructurePlaceSettings structurePlaceSettings = this.getSettings(
            rotation,
            boundingBox,
            liquidSettings,
            replaceJigsaws
    ...
    public StructureTemplate getTemplate(
        ...
        return this.template.map(
            structureTemplateManager::getOrCreate,
            Function.identity()
        );
    }
    ...
    private StructurePlaceSettings getSettings(
        ...
        structurePlaceSettings.setLiquidSettings(
            this.overrideLiquidSettings.orElse(liquidSettings)
        );
        if (!replaceJigsaws) {
            structurePlaceSettings.addProcessor(
    ...
    private List<StructureTemplate.StructureBlockInfo> getDataMarkers(
        StructureTemplateManager structureTemplateManager,
        BlockPos blockPos,
        Rotation rotation,
        boolean isPositionLocal
    ) {
        StructureTemplate structureTemplate = this.getTemplate(
            structureTemplateManager
        );
        ...
        List<StructureTemplate.StructureBlockInfo> dataBlocks =
            Lists.newArrayList();

        for (StructureTemplate.StructureBlockInfo block : structureBlocks) {
            StructureMode structureMode = StructureMode.valueOf(
                block.nbt().getStringOr("mode", "DATA")
            );
    ...
    public static <E extends YungJigsawSinglePoolElement> RecordCodecBuilder<
        E,
        Holder<StructureProcessorList>
    > processorsCodec() {
        return StructureProcessorType.LIST_CODEC.fieldOf(
            "processors"
        ).forGetter(element -> element.processors);
    }
    ...
    > overrideLiquidSettingsCodec() {
        return LiquidSettings.CODEC.optionalFieldOf(
            "override_liquid_settings"
        ).forGetter(element -> element.overrideLiquidSettings);
    }

    private static <T> DataResult<T> encodeTemplate(
        ...
        return !optional.isPresent()
            ...
            : Identifier.CODEC.encode(optional.get(), ops, template);
    }
}
