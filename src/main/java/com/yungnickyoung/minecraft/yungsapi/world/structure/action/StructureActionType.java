...
public interface StructureActionType<C extends StructureAction> {
    ...
    Map<Identifier, StructureActionType<?>> ACTION_TYPES_BY_NAME =
        new HashMap<>();
    Map<StructureActionType<?>, Identifier> NAME_BY_ACTION_TYPES =
        new HashMap<>();

    /* Codecs */
    Codec<StructureActionType<?>> ACTION_TYPE_CODEC = Identifier.CODEC.flatXmap(
        Identifier ->
            Optional.ofNullable(ACTION_TYPES_BY_NAME.get(Identifier))
                .map(DataResult::success)
                .orElseGet(() ->
                    DataResult.error(
                        () -> "Unknown structure action type: " + Identifier
                    )
                ),
        actionType ->
            Optional.of(NAME_BY_ACTION_TYPES.get(actionType))
                .map(DataResult::success)
                .orElseGet(() ->
                    DataResult.error(
                        () ->
                            "No ID found for structure action type " +
                            actionType +
                            ". Is it registered?"
                    )
                )
    ...
    Codec<StructureAction> ACTION_CODEC =
        StructureActionType.ACTION_TYPE_CODEC.dispatch(
            "type",
            StructureAction::type,
            StructureActionType::codec
        );

    /* Types. Add any new types here! */
    StructureActionType<TransformAction> TRANSFORM = register(
        "transform",
        TransformAction.CODEC
    );
    StructureActionType<DelayGenerationAction> DELAY_GENERATION = register(
        "delay_generation",
        DelayGenerationAction.CODEC
    );

    /**
     * Utility method for registering StructureActionTypes.
     */
    static <C extends StructureAction> StructureActionType<C> register(
     ...
     */
    private static <C extends StructureAction> StructureActionType<C> register(
        String id,
        MapCodec<C> codec
    ) {
        return register(
