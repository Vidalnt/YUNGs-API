...
public class PieceInRangeCondition extends StructureCondition {
    ...
    private static final Identifier ALL = Identifier.fromNamespaceAndPath(
        YungsApiCommon.MOD_ID,
        "all"
    );

    public static final MapCodec<PieceInRangeCondition> CODEC =
        RecordCodecBuilder.mapCodec(builder ->
            builder
                .group(
                    Identifier.CODEC.listOf()
                        .optionalFieldOf("pieces", new ArrayList<>())
                        .forGetter(conditon -> conditon.matchPieces),
                    Codec.INT.optionalFieldOf("above_range", 0).forGetter(
                        conditon -> conditon.aboveRange
                    ),
                    Codec.INT.optionalFieldOf("horizontal_range", 0).forGetter(
                        conditon -> conditon.horizontalRange
                    ),
                    Codec.INT.optionalFieldOf("below_range", 0).forGetter(
                        conditon -> conditon.belowRange
                    )
                )
    ...
    public PieceInRangeCondition(
        List<Identifier> pieces,
        int aboveRange,
        int horizontalRange,
        int belowRange
    ) {
    ...
    public boolean passes(StructureContext ctx) {
        ...
        if (templateManager == null) YungsApiCommon.LOGGER.error(
            "Missing required field 'structureTemplateManager' for piece_in_range condition!"
        );
        if (pieces == null) YungsApiCommon.LOGGER.error(
            "Missing required field 'pieces' for piece_in_range condition!"
        );
        if (pieceEntry == null) YungsApiCommon.LOGGER.error(
            "Missing required field 'pieceEntry' for piece_in_horizontal_direction condition!"
        );
        if (
            templateManager == null || pieces == null || pieceEntry == null
        ) return false;
        ...
        for (PieceEntry otherPieceEntry : pieces) {
            ...
            if (
                otherPiece.getElement() instanceof SinglePoolElement ||
                otherPiece.getElement() instanceof YungJigsawSinglePoolElement
            ) {
                // If otherPiece has the same bounding box as our starting piece, skip over it, as
                // it is likely just the same piece.
                if (
                    otherPiece.getBoundingBox().equals(piece.getBoundingBox())
                ) {
                    continue;
                }

                StructureTemplate otherStructureTemplate =
                    otherPiece.getElement() instanceof SinglePoolElement
                        ? (
                              (SinglePoolElementAccessor) otherPiece.getElement()
                          ).callGetTemplate(templateManager)
                        : (
                              (YungJigsawSinglePoolElement) otherPiece.getElement()
                ...
                for (Identifier matchPieceId : matchPieces) {
                    StructureTemplate structureTemplate =
                        templateManager.getOrCreate(matchPieceId);
                    if (
                        otherStructureTemplate == structureTemplate ||
                        matchPieceId.equals(ALL)
                    ) {
                        ...
                        if (
                            ...
                            !otherPiece
                                .getBoundingBox()
                                .intersects(piece.getBoundingBox())
                        ) {
                            return true;
                        }
                    }
