...
public class BlockStateRandomizer {
    ...
    public static final Codec<BlockStateRandomizer> CODEC =
        RecordCodecBuilder.create(instance ->
            instance
                .group(
                    Entry.CODEC.listOf()
                        .fieldOf("entries")
                        .forGetter(randomizer -> randomizer.entries),
                    BlockState.CODEC.fieldOf("defaultBlockState").forGetter(
                        randomizer -> randomizer.defaultBlockState
                    )
                )
    ...
    public CompoundTag saveTag() {
        ...
        compoundTag.putInt(
            "defaultBlockStateId",
            Block.BLOCK_STATE_REGISTRY.getId(this.defaultBlockState)
        );
        ...
        ListTag entriesTag = Util.make(new ListTag(), tag -> {
            this.entries.forEach(entry -> {
                ...
                entryTag.putInt(
                    "entryBlockStateId",
                    Block.BLOCK_STATE_REGISTRY.getId(entry.blockState)
                );
    ...
    public BlockStateRandomizer(CompoundTag compoundTag) {
        this.defaultBlockState = Block.BLOCK_STATE_REGISTRY.byId(
            compoundTag.getIntOr("defaultBlockStateId", 0)
        );
        ...
        entriesTag.forEach(entryTag -> {
            ...
            BlockState blockState = Block.BLOCK_STATE_REGISTRY.byId(
                entryCompoundTag.getIntOr("entryBlockStateId", 0)
            );
    ...
    public BlockStateRandomizer addBlock(BlockState blockState, float chance) {
        ...
        if (
            entries
                .stream()
                .anyMatch(entry -> entry.blockState.equals(blockState))
        ) {
            YungsApiCommon.LOGGER.warn(
                "WARNING: duplicate block {} added to BlockStateRandomizer!",
                blockState.toString()
            );
            return this;
        }
        ...
        if (newTotal > 1.0F) {
            // Total probability cannot exceed 1
            YungsApiCommon.LOGGER.warn(
                "WARNING: block {} added to BlockStateRandomizer exceeds max probabiltiy of 1!",
                blockState.toString()
            );
            return this;
        }
    ...
    public BlockState get(Random random) {
        ...
        for (Entry entry : entries) {
            if (
                currBottom <= target && target < currBottom + entry.probability
            ) {
    ...
    public BlockState get(RandomSource randomSource, StructureContext ctx) {
        ...
        for (Entry entry : entries) {
            if (
                currBottom <= target &&
                target < currBottom + entry.probability &&
                entry.passesCondition(ctx)
            ) {
                return entry.blockState;
            }
    ...
    public Map<BlockState, Float> getEntriesAsMap() {
        ...
        this.entries.forEach(entry ->
            map.put(entry.blockState, entry.probability)
        );
        return map;
    }
    ...
    public static class Entry {
        ...
        public static Codec<Entry> CODEC = RecordCodecBuilder.create(instance ->
            instance
                .group(
                    BlockState.CODEC.fieldOf("blockState").forGetter(entry ->
                        entry.blockState
                    ),
                    Codec.floatRange(0.0F, 1.0F)
                        .fieldOf("probability")
                        .forGetter(entry -> entry.probability),
                    StructureConditionType.CONDITION_CODEC.optionalFieldOf(
                        "condition"
                    ).forGetter(entry -> entry.condition)
                )
