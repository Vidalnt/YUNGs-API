 ...
 * </pre>
 */
public class AutoRegisterEntityType<
    T extends Entity
> extends AutoRegisterEntry<EntityType<T>> {

    private Supplier<AttributeSupplier.Builder> attributesBuilderSupplier;

    public static <U extends Entity> AutoRegisterEntityType<U> of(
        Supplier<EntityType<U>> entityTypeSupplier
    ) {
        return new AutoRegisterEntityType<>(entityTypeSupplier);
    }

    public AutoRegisterEntityType<T> attributes(
        Supplier<AttributeSupplier.Builder> attributesBuilderSupplier
    ) {
    ...
    public static class Builder<T extends Entity> {
        ...
        private EntityDimensions dimensions = EntityDimensions.scalable(
            0.6F,
            1.8F
        );
        ...
        public static <T extends Entity> Builder<T> of(
            EntityType.EntityFactory<T> entityFactory,
            MobCategory mobCategory
        ) {
        ...
        public Builder<T> allowedInPeaceful(boolean allowed) {
            this.allowedInPeaceful = allowed;
            return this;
        }

        public Builder<T> noLootTable() {
            this.lootTable = resourceKey -> Optional.empty();
            return this;
        }
        ...
        public EntityType<T> build(ResourceKey<EntityType<?>> resourceKey) {
            return new EntityType<>(
                ...
                this.updateInterval,
                this.descriptionId.apply(resourceKey),
                this.lootTable.apply(resourceKey),
