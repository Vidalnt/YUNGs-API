...
public class ItemRandomizer {
    ...
    public static final Codec<ItemRandomizer> CODEC = RecordCodecBuilder.create(
        instance ->
            instance
                .group(
                    Entry.CODEC.listOf()
                    ...
                    BuiltInRegistries.ITEM.byNameCodec()
                        .fieldOf("defaultItem")
                        .forGetter(randomizer -> randomizer.defaultItem)
                )
    ...
    public CompoundTag saveTag() {
        ...
        compoundTag.putInt(
            "defaultItemId",
            BuiltInRegistries.ITEM.getId(this.defaultItem)
        );
        ...
        ListTag entriesTag = Util.make(new ListTag(), tag -> {
            this.entries.forEach(entry -> {
                ...
                entryTag.putInt(
                    "entryItemId",
                    BuiltInRegistries.ITEM.getId(entry.item)
                );
    ...
    public ItemRandomizer(CompoundTag compoundTag) {
        this.defaultItem = BuiltInRegistries.ITEM.byId(
            compoundTag.getIntOr("defaultItemId", 0)
        );
        ...
        entriesTag.forEach(entryTag -> {
            ...
            Item item = BuiltInRegistries.ITEM.byId(
                entryCompoundTag.getIntOr("entryItemId", 0)
            );
    ...
    public ItemRandomizer addItem(Item item, float chance) {
        ...
        if (entries.stream().anyMatch(entry -> entry.item.equals(item))) {
            YungsApiCommon.LOGGER.warn(
                "WARNING: duplicate item {} added to ItemRandomizer!",
                item.toString()
            );
            return this;
        }
        ...
        if (newTotal > 1) {
            ...
            YungsApiCommon.LOGGER.warn(
                "WARNING: item {} added to ItemRandomizer exceeds max probabiltiy of 1!",
                item.toString()
            );
            return this;
        }
