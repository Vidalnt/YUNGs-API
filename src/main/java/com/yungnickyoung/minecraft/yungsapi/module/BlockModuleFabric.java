...
public class BlockModuleFabric {
    ...
    private static void register(AutoRegisterField data) {
        ...
        if (autoRegisterBlock.hasItemProperties()) {
            ItemModuleFabric.registerBlockItem(
                data.name(),
                block,
                autoRegisterBlock.getItemProperties().get()
            );
        }
        ...
        if (autoRegisterBlock.hasStairs()) {
            Block stairBlock = StairBlockAccessor.createStairBlock(
                block.defaultBlockState(),
                BlockBehaviour.Properties.ofLegacyCopy(block)
            );
            Identifier name = Identifier.fromNamespaceAndPath(
                namespace,
                path + "_stairs"
            );
        ...
        if (autoRegisterBlock.hasFence()) {
            Block fenceBlock = new FenceBlock(
                BlockBehaviour.Properties.ofLegacyCopy(block)
            );
            Identifier name = Identifier.fromNamespaceAndPath(
                namespace,
        ...
        if (autoRegisterBlock.hasFenceGate()) {
            Block fenceGateBlock = new FenceGateBlock(
                autoRegisterBlock.getFenceGateWoodType(),
                BlockBehaviour.Properties.ofLegacyCopy(block)
            );
            Identifier name = Identifier.fromNamespaceAndPath(
                namespace,
